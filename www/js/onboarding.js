...
async function showTextModal(title, type) {
  ...
  const content = type === 'terms' ? getTermsHTML() : getPrivacyHTML();
...
function getPrivacyHTML() {
  return `
    ...
    <p>All data you enter into Neyra remains on your device, including mood entries, notes (text, voice, images), practice usage data, and insights generated by the app.</p>
    <p style="margin-top:8px;">This data is not transmitted anywhere, not accessible to the developer, and not used for tracking or profiling.</p>
    ...
    <p>Neyra itself does not share your data. Certain technical data may be processed by platform providers required to deliver the app, such as Google Play (app distribution and subscription payments).</p>
    ...
    <p>All data is stored locally on your device. You can delete the app at any time to remove your data. Local backup and restore features are available.</p>

    <h3 style="margin:20px 0 8px;color:#3a3530;">8. Your Rights (EU Users)</h3>
    <p>If you are located in the European Union, you have rights under GDPR, including access and deletion. Since Neyra does not store data externally, you can fully control your data directly on your device.</p>
